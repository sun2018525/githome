<template>
  <el-dialog v-model="visible" :title="!dataForm.id ? '新增' : '修改'" :close-on-click-modal="false" :close-on-press-escape="false">
    <el-form :model="dataForm" :rules="rules" ref="dataFormRef" @keyup.enter="dataFormSubmitHandle()" label-width="150px">
      ...
      <el-form-item prop="officeId" label="科室">
        <el-select v-model="dataForm.officeId"  placeholder="请选择科室">
          <el-option
      ...
      <el-form-item prop="doctorInfo" label="医生简介">
        <el-input type="textarea" v-model="dataForm.doctorInfo" placeholder="医生简介"></el-input>
      </el-form-item>
...
const rules = ref({
  ...
  officeId: [{ required: true, message: "必填项不能为空", trigger: "change" }],
...
const init = (id?: number) => {
  ...
  Promise.all([getOfficeList()]).then(() => {
    if (id) {
      getInfo(id);
    }
  });
